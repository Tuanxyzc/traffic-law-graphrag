...
class RewrittenQuery(BaseModel):
    ...
    must_have_terms: list[str] = Field(
        ...
        description="Mandatory statutory terms that must be present in retrieved chunks",
    )
    must_not_have_terms: list[str] = Field(
        default_factory=list,
        description="Discriminative negative terms that must not collide with current violation",
    )
    target_entities: list[str] = Field(
        default_factory=list,
        description="Target vehicle categories (e.g. 'xe_o_to', 'xe_mo_to')",
    )
    intent: Literal[
        "violation_sanction",
        "document_amendment",
    ...
    def to_json_dict(self) -> dict[str, Any]:
        """Returns the standardized JSON dictionary for output and logging."""
        return {
            "intent": self.intent,
            ...
            "sanction_query": self.sanction_query,
            "identified_keywords": self.identified_keywords,
            "must_have_terms": self.must_have_terms,
            "must_not_have_terms": self.must_not_have_terms,
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, str):
            return self.search_query == other
...
class EvidencePackage(BaseModel):
    ...
    rewritten_query: RewrittenQuery | str = Field(
        ..., description="Statutory search query with full rewrite decomposition"
    )
...
class GenerationResult(BaseModel):
    ...
    grounding_verified: bool = Field(
        default=True, description="True if answer passed action grounding guard"
    )
...
class PipelineResult(BaseModel):
    ...
    rewritten_query: RewrittenQuery | str = Field(
        ..., description="Formal legal search query with full rewrite decomposition"
    )
