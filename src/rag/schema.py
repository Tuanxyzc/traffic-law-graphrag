...
class RAGSchemaManager:
    ...
    def ensure_schema(self, session: Session) -> list[str]:
        """Executes Cypher statements ensuring constraint, vector index, and fulltext index exist."""
        executed: list[str] = []
    ...
    def wait_for_indexes(
        ...
        """Waits until both vector and fulltext indexes transition to 'ONLINE' state."""
        start_time = time.time()
        expected = {self.config.vector_index_name, self.config.fulltext_index_name}
