...
from src.graph.identity import make_article_id,make_clause_id,make_point_id,make_chapter_id
from src.graph.models import GraphRelationship,GraphNode
...
def map_chapter(document_id: str, chapter_data: dict):
    ...
    node = GraphNode(
        ...
        properties = {
            "number":so,
            "title": chapter_data.get("tieu_de","")
        }
    )

    relationship = GraphRelationship(
        start_id=document_id,
        relationship_type="CONTAINS_CHAPTER",
        end_id=chapter_id
    )
...
def map_chapter_articles(document_id: str, chapter_data: dict):
    ...
    for article_data in chapter_data.get("dieu", []):
        article_nodes,article_relationships = map_article(document_id,article_data,chapter_id)
        nodes.append(article_nodes)
        relationships.append(article_relationships)

        for clause_data in article_data.get("khoan", []):
            clause_nodes,clause_relationships = map_clause(document_id, article_data["so"], clause_data)
            nodes.append(clause_nodes)
            relationships.append(clause_relationships)
            
            for point_data in clause_data.get("diem", []):
                point_nodes,point_relationships = map_point(document_id, article_data["so"], clause_data["so"], point_data)
...
def map_clause(document_id:str, article_number:str, clause_data: dict):
        ...
        node = GraphNode(
            ...
            properties={
                "number": number,
                "content": clause_data.get("noi_dung","") 
            }
        )
...
def map_point(document_id: str, article_number:str, clause_number: str, point_data: dict):
        ...
        node = GraphNode(
            ...
            properties={
                "number": number,
                "content": point_data.get("noi_dung","")
            }
        )
        relationship = GraphRelationship(
...
def map_document_structure(document_data: dict, resolver):
    ...
    for chapter_data in document_data.get("chuong", []):
        ...
        chapter_nodes, chapter_relationships = map_chapter_articles(document_id, chapter_data)
    ...
    for article_data in document_data.get("dieu_khong_chuong",[]):
        ...
        for clause_data in article_data.get("khoan",[]):
            clause_node,clause_relationship=map_clause(document_id,str(article_data["so"]),clause_data)
            nodes.append(clause_node)
            relationships.append(clause_relationship)
            for point_data in clause_data.get("diem",[]):
                point_node,point_relationship=map_point(document_id,str(article_data["so"]),str(clause_data["so"]),point_data)
