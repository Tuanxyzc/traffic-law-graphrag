from src.graph.amendment.amendment_mapper import (
    get_sua_doi_replacement
)
...
def validate_sua_doi_replacement_levels(action): 
    ...
    for target in action.get("targets", []):
        ...
        if target_level != replacement_level: 
            raise ValueError( "SUA_DOI target_level != " "replacement_level: " 
            f"{target.get('target_unit')}" ) 
    return True
def validate_sua_doi_replacement_node_levels(
    action,
    replacements,
    replacement_nodes
):
    ...
    for node in replacement_nodes:
        ...
        target_unit = node.properties.get(
            "target_unit"
        )

        actual_level = node.properties.get(
            "replacement_level"
        )
    ...
    return True
def validate_sua_doi_replacements(
    item,
    action,
    resolver
):
    ...
    replacements = get_sua_doi_replacement(
        item,
        action,
        resolver
    )
...
def validate_sua_doi_replacement_paths(
    action,
    replacements
):
    ...
    if len(targets) != len(replacements):
        raise ValueError(
            "Target/replacement count mismatch"
        )
    ...
    for target, replacement in zip(
        ...
        target_unit = target.get(
            "target_unit"
        )

        replacement_target = replacement.get(
            "target_unit"
        )
        ...
        replacement_path = replacement.get(
            "replacement_path"
        )

        if not replacement_path:
            raise ValueError(
                f"SUA_DOI target {target_unit} "
        ...
        context_node = (target.get("target_context") or {}).get(level_key) if level_key else None
...
def validate_sua_doi_graph(
    action_node,
    relationships,
    replacement_nodes
):

    if action_node.properties.get(
        "operation"
    ) != "SUA_DOI":
        return True
    ...
    amend_targets = [
        ...
        if (
            r.start_id == action_id
            and r.relationship_type == "AMENDS"
        )
    ]
...
def validate_amendment_graph(
    ...
    try:
        action_index = next(i for i, candidate in enumerate(actions, start=1)
    ...
    if len(action_nodes) != 1:
        raise ValueError(
            "Expected exactly 1 AmendmentAction node, "
            f"found {len(action_nodes)}"
        )
    ...
    replacement_nodes = [
        node
        for node in nodes
        if node.label == "AmendmentReplacement" and node.id.startswith(expected_action_id + "_R")
    ]
    ...
    validate_sua_doi_replacement_levels(
        action
    )

    validate_sua_doi_replacements(
        item,
        action,
        resolver
    )
    ...
    validate_sua_doi_replacement_paths(
        action,
        replacements
    )

    validate_sua_doi_graph(
        action_node,
        relationships,
        replacement_nodes
    )
