...
def validate_cross_document(
    ...
    for document_id, result in amendment_results.items():
        ...
        validate_amends_targets(
            relationships,
            structure_node_index
        )

        validate_repeals_targets(
            relationships,
            structure_node_index
        )

        validate_applies_to_targets(
            relationships,
            structure_node_index
        )
...
def validate_global_node_duplicates(
    nodes
):
...
def validate_global_relationship_duplicates(
    relationships
):
...
def validate_global_relationship_references(
    nodes,
    relationships
):
    ...
    for rel in relationships:
        ...
        if rel.start_id not in node_ids:
            raise ValueError(
                f"Global relationship start "
                f"not found: {rel.start_id}"
            )

        if rel.end_id not in node_ids:
            raise ValueError(
                f"Global relationship end "
                f"not found: {rel.end_id}"
            )

    return True


def validate_global_graph(
    nodes,
    relationships
):

    validate_global_node_duplicates(
        nodes
    )

    validate_global_relationship_duplicates(
        relationships
    )

    validate_global_relationship_references(
        nodes,
        relationships
    )
