...
from src.graph.identity import make_point_id,make_clause_id,make_article_id
import json
...
def map_amendment_item(
    ...
    semantic_node, semantic_rel = map_semantic_unit(
        item,
        semantic_index
    )
...
#UTILITIES
def map_target_relationship(action_node_id: str, action: dict, relationship_type: str, resolver):
    relationships = []
...
def map_amendment_replacement(
    ...
    replacement_id = (
        f"{action_id}_R{replacement_index}"
    )
    ...
    node = GraphNode(
        ...
        properties={
            ...
            "replacement_level": replacement_info[
                "replacement_level"
            ],
...
def map_sua_doi(item, action, semantic_node, action_index, resolver):
    ...
    action_rel = GraphRelationship(
        start_id=semantic_node.id,
        relationship_type="HAS_ACTION",
        end_id=action_node.id
    )
    ...
    for index, replacement_info in enumerate(replacements,start=1):
        replacement_node = map_amendment_replacement(action_node.id, index, replacement_info)
...
def map_bai_bo(item,action,semantic_node,action_index, resolver):
    ...
    target_relationship = map_target_relationship(action_node.id, action, "REPEALS", resolver)
    relationships.extend(target_relationship)
    return nodes,relationships

#==BỔ SUNG============================================================================================
def map_created_unit(
    item: dict,
    created_unit: dict,
    resolver
):
    ...
    replacement = map_replacement_unit(
        item,
        created_unit
    )
    ...
    if unit_level == "ARTICLE":
        ...
        child_nodes, child_rel = (
            map_article_children(
                article_node,
                document_id,
                article_number,
                replacement
...
def map_anchor_relationship(action_node_id: str, action: dict, resolver):
    ...
    if point is not None:
        if clause is None:
             raise ValueError(
                "Anchor POINT nhưng thiếu clause")
...
def map_bo_sung(
    ...
    for created_unit in action.get("created_units",[]):
        ...
        if created_root_node is None:
            raise ValueError(
                f"Không tạo được created unit: "
                f"{created_unit.get('unit_id')}"
            )
...
def map_article_children(article_node, document_id, article_number, replacement_article):
    nodes = []
    relationships= []
    ...
    for clause_data in replacement_article.get("clauses",[]):
        ...
        point_nodes, point_relationships = map_clause_children(clause_node,document_id,clause_number,article_number,clause_data)
        nodes.extend(point_nodes)
        relationships.extend(point_relationships)

    return nodes,relationships

def map_clause_children(clause_node,document,clause_number,article_number,clause_data):
    nodes = []
    relationships = []
...
def map_amendment_text(action_id: str, amendment_text: dict):
    ...
    properties = {
        "unit_type": amendment_text.get("unit_type")
    }
...
def map_text_amendment_operation(
    ...
    if not text_amendment:
        raise ValueError(
            ...
            "không có text_amendment"
        )

    text_node = map_amendment_text(
        action_node.id,
        text_amendment
    )
...
#=================================================APPENDIX============================================================================================
def map_amendment_appendix(
    action_id: str,
    appendix_amendment: dict
):

    appendix_id = f"{action_id}_APPENDIX"

    old_appendix = appendix_amendment.get(
        "old_appendix",
        {}
    )

    new_appendix = appendix_amendment.get(
        "new_appendix",
        {}
    )
...
def map_thay_the_phu_luc(
    ...
    if not appendix_amendment:
        raise ValueError(
            "THAY_THE_PHU_LUC "
            "không có appendix_amendment"
        )

    appendix_node = map_amendment_appendix(
        action_node.id,
        appendix_amendment
    )
