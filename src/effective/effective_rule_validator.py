...
def validate_effective_rule_schema(rule: dict):
    ...
    if missing:
        raise ValueError(f"Effective rule: {rule['rule_id']}Missing fields: {missing}")
...
def validate_effective_rule_dates(rule: dict):
    ...
    if (
        ...
        raise ValueError(f"Invalid effective interval: {rule['rule_id']}")
...
def validate_external_rule(rule):
    ...
    if not rule.get("external_basis"):
        raise ValueError(f"EXTERNAL_RULE missing external_basis: {rule['rule_id']}")

    if rule.get("status") != "EXTERNAL":
        raise ValueError(f"EXTERNAL_RULE invalid status: {rule['rule_id']}")
...
def validate_effective_rule_targets(rule, structure_node_index, resolver):
    ...
    if target_document not in structure_node_index:
        raise ValueError(f"Target document not found: {target_document}")
    ...
    if rule_type == "GENERAL":
        ...
        for target in rule.get("targets", []):
            if not target:
                raise ValueError(
                    f"Rule {rule_type} rule has no targets: {rule['rule_id']}"
                )

            unit_id = resolver.resolve_unit(target.get("unit_id"))
            if not unit_id:
                raise ValueError(f"Target missing unit_id: {rule['rule_id']}")
            if unit_id not in structure_node_index:
                raise ValueError(
                    f"Target unit not found: {unit_id} in {target_document}"
                )
    else:
...
def validate_effective_rule_duplicates(rules, resolver):
    ...
    for rule in rules:
        ...
        if key in seen:
            raise ValueError(f"Duplicate effective rule: {rule['rule_id']}")
...
def validate_effective_rules(rules, structure_node_index, resolver):

    for rule in rules:
        validate_effective_rule_schema(rule)

        validate_effective_rule_dates(rule)

        validate_external_rule(rule)

        validate_effective_rule_targets(rule, structure_node_index, resolver)

    validate_effective_rule_duplicates(rules, resolver)
