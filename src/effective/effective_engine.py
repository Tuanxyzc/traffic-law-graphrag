...
def is_rule_active(rule: dict, as_of_date: date)-> bool:
    ...
    and as_of_date < effective_from):
        return False
    
    if (effective_to is not None 
    and as_of_date >= effective_to):
        return False
...
def get_applicable_rules(
    ...
    for rule in rules:
        ...
        for target in rule.get(
            ...
            if resolver.resolve_unit(target.get(
                "unit_id"
            )) == target_unit:
                applicable.append(rule)
                break
...
def evaluate_unit(
    ...
    target_rules = get_rules_for_target(
        rules,
        target_unit,
        resolver
    )
    ...
    if external_rules:
        return EffectiveResult(
            ...
            effective_from=None,
            effective_to=None,
            source_unit=external_rules[0].get(
                "source_unit"
            ),
            external_basis=external_rules[0].get(
                "external_basis"
            )
        )
    ...
    if active_rules:
        ...
        return EffectiveResult(
            ...
            target_unit=target_unit,
            effective_from=rule.get(
                "effective_from"
            ),
            effective_to=rule.get(
                "effective_to"
            ),
            source_unit=rule.get(
                "source_unit"
            ),
            external_basis=None
        )
