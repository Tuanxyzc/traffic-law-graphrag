...
def _semantic_owns_event(semantic_id: str, event_source_id: str) -> bool:
    ...
    return event_source_id == semantic_id or event_source_id.startswith(
        semantic_id + "_D"
    )
...
def sync_semantic_amendments(root: Path):
    ...
    for document, events in by_document.items():
        ...
        for unit in units:
            owned = [
                e for e in events if _semantic_owns_event(unit["id"], e["source_unit"])
            ]
            if not owned:
                continue
            operations = sorted(
            ...
            unit["hanh_dong"] = (
                operations[0] if operations else unit.get("hanh_dong", "GIU_NGUYEN")
            )
...
def mark_external_references(root: Path):
    ...
    for path in sorted(root.glob("*_reference_index.json")):
        ...
        _write(path, edges)
    _write(
        root / "external_nodes.json",
        sorted(external_nodes.values(), key=lambda x: x["id"]),
    )
    ...
    for path in root.glob("*_effective_rules.json"):
        ...
        for rule in payload.get("rules", []):
            ...
            if source_id not in selected_ids:
                support[source_id] = {
                    "id": source_id,
                    "node_type": "EFFECTIVE_RULE_SUPPORT",
            ...
            for target in rule.get("targets", []):
                ...
                if target_id and target_id not in selected_ids:
                    support.setdefault(
                        ...
                        {
                            "id": target_id,
                            "node_type": "EFFECTIVE_RULE_TARGET_SUPPORT",
                            "document": rule.get("target_document")
        ...
        _write(path, payload)
    _write(
        root / "effective_support_nodes.json",
        sorted(support.values(), key=lambda x: x["id"]),
    )
            ...
            if isinstance(value, dict):
                ...
                if old:
                    ...
                    if counts[old] > 1:
                        ...
                        report.append(
                            {"file": path.name, "original_id": old, "new_id": new}
                        )
