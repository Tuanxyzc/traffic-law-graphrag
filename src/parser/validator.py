...
def validate_article_number(van_ban: VanBan) -> list[str]:
    ...
    if missing:
        warnings.append(f"[{van_ban.so_hieu}] Số Điều bị thiếu (không liên tục): {missing}")
    return warnings
...
def validate_clause(van_ban: VanBan) -> list[str]:
    ...
    for d in _all_dieu(van_ban):
        ...
        if dup:
            warnings.append(f"[{van_ban.so_hieu}] Điều {d.so}: Khoản bị TRÙNG số {dup} — kiểm tra lại có bị lẫn số lồng không")
    return warnings
...
def validate_point(van_ban: VanBan) -> list[str]:
    ...
    for d in _all_dieu(van_ban):
        for k in d.khoan:
            ...
            dup = {x for x in so_list if so_list.count(x) > 1}
            if dup:
                warnings.append(f"[{van_ban.so_hieu}] Điều {d.so} Khoản {k.so}: Điểm bị TRÙNG số {dup}")
    return warnings
    ...
    for u in units:
        for tc in u.tham_chieu:
            if not tc.gia_tri_xac_dinh.so_hieu_van_ban:
                warnings.append(f"[{u.id}] Tham chiếu '{tc.van_ban_goc}' không resolve được văn bản")
    return warnings
    ...
    for c in van_ban.chuong:
        for d in c.dieu:
            if d.id_cha and d.id_cha not in all_ids:
                warnings.append(f"[{van_ban.so_hieu}] Điều {d.id}: id_cha '{d.id_cha}' không tồn tại")
    for d in _all_dieu(van_ban):
        for k in d.khoan:
            if k.id_cha not in all_ids:
                warnings.append(f"[{van_ban.so_hieu}] Khoản {k.id}: id_cha '{k.id_cha}' không tồn tại")
            for p in k.diem:
                if p.id_cha not in all_ids:
                    warnings.append(f"[{van_ban.so_hieu}] Điểm {p.id}: id_cha '{p.id_cha}' không tồn tại")
    return warnings
...
def validate_invalid_reference(units, index_so_hieu_to_ids: dict[str, set[str]] | None) -> list[str]:
    """
    ...
    for u in units:
        for tc in u.tham_chieu:
            ...
            if known_ids is not None and tid not in known_ids:
                warnings.append(f"[{u.id}] Tham chiếu '{tc.van_ban_goc}' -> '{tid}' KHÔNG tồn tại trong {vt.so_hieu_van_ban}")
    return warnings


def build_global_index(parsed_dir: str, exclude_so_hieu: str | None = None) -> dict[str, set[str]]:
    """
...
def validate(van_ban: VanBan, units=None, index_so_hieu_to_ids: dict[str, set[str]] | None = None) -> list[str]:
    ...
    for event in events:
        for item in event["items"]:
            for act in item.get("actions", []):
                ...
                # Check duplicates
                t_units = [t.get("target_unit") for t in targets if t.get("target_unit")]
                dups = set([x for x in t_units if t_units.count(x) > 1])
                if dups:
                    warnings.append(f"Validation Error: Duplicate targets found in action: {dups}")
                ...
                for target in targets:
                    ...
                    if op == "BAI_BO" and repl_path is not None:
                        warnings.append(f"Validation Error: BAI_BO cannot have a replacement path. Found: {repl_path}")
                    if op in ["SUA_DOI", "BO_SUNG", "THAY_THE"] and repl_path is None:
                        warnings.append(f"Validation Error: {op} must have a valid replacement path mapped to a tree for target {target.get('target_unit')}")
                    
                    # Check levels
                    if target.get("target_level") and target.get("replacement_level"):
                        if target["target_level"] != target["replacement_level"]:
                            warnings.append(f"Validation Error: Target Level ({target['target_level']}) does not match Replacement Level ({target['replacement_level']})")
                
                # Replacement paths should be unique
                if op == "SUA_DOI":
                    path_dups = set([x for x in repl_paths if repl_paths.count(x) > 1])
                    if path_dups:
                        warnings.append(f"Validation Error: Duplicate replacement paths found in SUA_DOI: {path_dups}")
    return warnings
