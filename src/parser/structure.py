...
from src.parser.models import VanBan, Chuong, Dieu, Khoan, Diem
...
def parse_diem_list(text: str, id_cha: str, initial_quote_balance: int = 0, initial_straight_quote: bool = False) -> list[Diem]:
    ...
    for line in lines:
        is_inside_quote = quote_balance > 0 or inside_straight_quote or line.lstrip().startswith("“") or line.lstrip().startswith('"')
...
def parse_khoan_list(text: str, id_cha: str, initial_quote_balance: int = 0, initial_straight_quote: bool = False) -> list[Khoan]:
    ...
    def flush_khoan():
        if current_noi_dung and current_khoan_so:
            ...
            nd = "\n".join(current_noi_dung).strip()
            result.append(Khoan(
                id=khoan_id, id_cha=id_cha, so=current_khoan_so, noi_dung=nd,
...
def parse_document(paragraphs: list[DoanVan], so_hieu: str, ten: str = "", loai: str = "") -> VanBan:
    """
    ...
    quote_balance = 0  # >0 = đang ở trong khối "..." — KHÔNG coi là ranh giới Điều cấp 1
    ...
    for p in paragraphs:
        ...
        is_inside_quote = (quote_balance > 0) or inside_straight_quote or text.startswith("“") or text.startswith('"')
        ...
        if APPENDIX_HEADING.match(text.strip()) and section_state == SECTION_DOCUMENT_BODY:
            flush_dieu()
            section_state = SECTION_APPENDIX
        elif SIGNATURE_HEADING.match(text.strip()) and section_state == SECTION_DOCUMENT_BODY:
            flush_dieu()
            section_state = SECTION_SIGNATURE
        elif RECIPIENT_HEADING.match(text.strip()) and section_state == SECTION_DOCUMENT_BODY:
            flush_dieu()
            section_state = SECTION_RECIPIENT

        dieu_match = DIEU_TITLE_PATTERN.match(text)
        if dieu_match and p.dam and not is_inside_quote and section_state == SECTION_DOCUMENT_BODY:
            flush_dieu()
