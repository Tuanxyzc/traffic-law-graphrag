...
def run(parsed_dir="data/parsed"):
            ...
            for item in event.get("items", []):
                for action in item.get("actions", []):
                    for target in action.get("targets", []):
                        ...
                        if target_id:
                            event_targets.setdefault(
                                item.get("source_unit"), set()
                            ).add(target_id)
        ...
        for unit in _read(root / f"{doc}_semantic_units.json"):
            ...
            for target in unit.get("doi_tuong", []):
                if target.get("dieu") and target.get("so_hieu_van_ban"):
                    target_id = (
                        target["so_hieu_van_ban"].replace("/", "_")
                        + "_D"
                        + target["dieu"]
                    )
                    if target.get("khoan"):
    ...
    report = {
        "issue_1_backfill_originals": {
            "pass": backfilled_units > 0,
            "marked_units": backfilled_units,
        },
        "issue_2_amendment_edges": {
            "pass": bool(amendment_edges),
            "edge_count": len(amendment_edges),
        },
        ...
        "issue_4_quoted_context": {
            "pass": correct_184_target and not wrong_184_target,
            "correct_edge_present": correct_184_target,
            "wrong_edges": wrong_184_target,
        },
        "issue_5_deduplication": {
            "pass": not duplicate_references,
            "duplicates": duplicate_references,
        },
        "issue_6_effective_support": {
            "pass": bool(support_nodes),
            "support_node_count": len(support_nodes),
        },
        "issue_7_duplicate_ids": {"pass": len(id_repairs) >= 2, "repairs": id_repairs},
        "issue_8_external_references": {
            "pass": external_edges > 0 and bool(external_nodes),
    ...
    report["all_pass"] = all(
        item["pass"] for key, item in report.items() if key.startswith("issue_")
    )
