...
class Diem:
    ...
    cap: int = (
        5  # level trong cây Document(1)->Chapter(2)->Article(3)->Clause(4)->Point(5)
    )
...
class Khoan:
    ...
    hieu_luc_tu: str | None = (
        None  # override ngày hiệu lực riêng (nếu khác văn bản sửa đổi)
    )
...
class Dieu:
    ...
    id_cha: (
        str | None
    )  # id của Chương chứa Điều này (None nếu văn bản không chia chương)
    so: str
    tieu_de: str
    so_hieu_van_ban: str  # văn bản mà Điều này thuộc về (khóa chuẩn hóa)
    noi_dung: str = (
        ""  # toàn bộ văn bản của Điều (dùng để lưu trữ nếu không có khoản/điểm)
    )
...
class VanBan:
    ...
    dieu_khong_chuong: list[Dieu] = field(
        default_factory=list
    )  # nếu văn bản không chia chương
    cap: int = 1
...
class DonViNguNghia:
    ...
    doi_tuong: list[ViTri] = field(
        default_factory=list
    )  # vị trí ĐÍCH bị tác động (dùng cho THAY_THE/BAI_BO nhiều vị trí)
    tham_chieu: list[ThamChieu] = field(default_factory=list)
    noi_dung_goc: str = ""  # nguyên văn, CHƯA thay "Điều này"/"khoản này"...
    noi_dung_chuan_hoa: str = (
        ""  # đã thay thế mọi tham chiếu tương đối bằng số tuyệt đối
    )
...
def van_ban_from_old_flat_list(
    data: list[dict], so_hieu: str, ten: str = "", loai: str = ""
) -> VanBan:
    """
