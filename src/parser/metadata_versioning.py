...
EFFECT_FROM_RE = re.compile(
    r"(?:có|được)\s+hiệu\s+lực(?:\s+thi\s+hành)?\s+(?:từ|kể\s+từ)\s+ngày\s+(\d{1,2})\s+tháng\s+(\d{1,2})\s+năm\s+(\d{4})",
    re.IGNORECASE,
)
EFFECT_TO_RE = re.compile(
    r"hết\s+hiệu\s+lực\s+(?:kể\s+từ|từ)\s+ngày\s+(\d{1,2})\s+tháng\s+(\d{1,2})\s+năm\s+(\d{4})",
    re.IGNORECASE,
)
...
def extract_header_metadata(source_file: str) -> dict:
    ...
    for text in header_texts:
        ...
        if first in ("QUỐC HỘI", "CHÍNH PHỦ") or first.startswith(
            ("BỘ ", "THỦ TƯỚNG CHÍNH PHỦ")
        ):
            authority = first
            break
...
def build_effective_rules(van_ban: VanBan) -> dict:
    ...
    for article in _articles(van_ban):
        ...
        if not sources and article.noi_dung:
            sources = [
                type("RuleSource", (), {"so": None, "noi_dung": article.noi_dung})()
            ]
        for source in sources:
            ...
            for part_index, part in enumerate(
                ...
                external = (
                    bool(
                        re.search(
                            r"hiệu lực.*theo quy định của pháp luật",
                            part,
                            re.IGNORECASE,
                        )
                    )
                ...
                rule_type = (
                    "EXTERNAL_RULE"
                    if external
                    else ("GENERAL" if is_general else "EXPLICIT")
                )
                rules.append(
                    {
...
def save_effective_rules(van_ban: VanBan, output_dir: str):
    ...
    path = (
        Path(output_dir)
        / f"{canonical_document_id(van_ban.so_hieu)}_effective_rules.json"
    )
