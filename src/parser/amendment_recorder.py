...
from src.parser.normalize import DoanVan, load_docx, normalize_paragraphs
from src.parser.structure import (
    DIEM_PATTERN,
    DIEU_TITLE_PATTERN,
    KHOAN_PATTERN,
    parse_diem_list,
...
def build_replacement_tree_and_root(
    quote_block: str, root_level: str
) -> tuple[dict, str]:
    if root_level == "POINT":
        pts = parse_diem_list(quote_block, "tmp")
        if len(pts) > 1:
            return {
                "points": [{"number": p.so, "content": p.noi_dung} for p in pts]
            }, "POINT_LIST"
        elif len(pts) == 1:
        ...
        return {
            "point": {
                "number": quote_block[:1]
                if len(quote_block) > 1 and quote_block[1] == ")"
                else None,
                "content": quote_block.strip(),
            }
        }, "POINT"
    ...
    if root_level == "CLAUSE":
                    ...
                    "clauses": [
                        {
                            ...
                            "content": k.noi_dung,
                            "points": [
                                {"number": p.so, "content": p.noi_dung} for p in k.diem
                            ],
                        }
                        for k in khoan_list
                    ]
        ...
        return {
            "clause": {"number": None, "content": quote_block, "points": []}
        }, "CLAUSE"
    ...
    if current_article:
        article_blocks.append("".join(current_article).strip())
    if len(article_blocks) > 1 and all(
        DIEU_TITLE_PATTERN.match(block.splitlines()[0]) for block in article_blocks
    ):
        articles = [
            build_replacement_tree_and_root(block, "ARTICLE")[0]["article"]
...
def parse_amendment_workflow(paragraphs) -> list[dict]:
    ...
    for p in paragraphs:
        ...
        if state == State.NORMAL:
            ...
            is_text_amend_inst = bool(
                re.search(
                    r"(bổ\s+sung|bãi\s+bỏ|thay\s+thế|bỏ)\s+(?:một\s+số\s+)?(từ|cụm\s+từ|phụ\s+lục)",
                    text,
                    re.IGNORECASE,
                )
            )
...
def build_ids_for_context(
    so_hieu: str, context: dict, dieu=None, khoan=None, diem=None
):
    ...
    if context.get("article") and context["article"]["number"]:
        context["article"]["id"] = (
            f"{actual_so_hieu.replace('/', '_')}_D{context['article']['number']}"
        )
        if context.get("clause") and context["clause"]["number"]:
            context["clause"]["id"] = (
                f"{context['article']['id']}_K{context['clause']['number']}"
            )
            if context.get("point") and context["point"]["number"]:
                context["point"]["id"] = (
                    f"{context['clause']['id']}_D{context['point']['number']}"
                )
...
def parse_actions(text: str, default_so_hieu: str):
    ...
    for seg in segments:
        ...
        if hanh_dong == "THAY_THE_TEXT":
            m = re.search(
                r'thay thế\s+(?:một\s+số\s+)?(từ|cụm\s+từ)\s*["“”](.*?)["”]\s*bằng\s*(?:từ|cụm\s+từ)\s*["“”](.*?)["”]',
                seg,
                re.IGNORECASE,
            )
            if m:
            ...
            m = re.search(
                r'bổ sung\s+(từ|cụm\s+từ)\s*["“”](.*?)["”]\s*(vào\s+sau|sau|vào\s+trước|trước)\s*(?:từ|cụm\s+từ|từ\s+ngữ)\s*["“”](.*?)["”]',
                seg,
                re.IGNORECASE,
            )
            if m:
            ...
            if m:
                ...
                appendix_amend = {
                    ...
                    "new_appendix": {
                        "number": m.group(3).replace("Phụ lục", "").strip()
                        if m.group(3)
                        else m.group(1).replace("Phụ lục", "").strip(),
                        "document": None,  # Will be set to source_document by caller
                    },
                }

        anchor_match = re.search(
            r"(vào sau|vào trước|trước|sau|cuối)\s+(khoản|điểm|điều)\s+(.*?)(?=\s+như\s+sau|\s+và|$|,)",
            raw_instruction,
            re.IGNORECASE,
        )
        anchor_span = anchor_match.span() if anchor_match else None
        if anchor_match and hanh_dong not in (
            "BO_SUNG_TEXT",
            "THAY_THE_TEXT",
            "BAI_BO_TEXT",
            "THAY_THE_PHU_LUC",
            ...
            rel_enum = (
                "AFTER"
                if "sau" in rel_str
                else ("BEFORE" if "trước" in rel_str else "APPEND")
            )
            ...
            masked_instruction = (
                raw_instruction[: anchor_match.start()]
                + " " * (anchor_match.end() - anchor_match.start())
                + raw_instruction[anchor_match.end() :]
            )
        else:
        ...
        if hanh_dong == "THAY_THE_PHU_LUC":
            ...
            target_source = (
                mask_quoted_spans(raw_instruction)
            ...
            if shared_article and not re.search(
                r"\bĐiều\s+\d+[a-zđ]?", contextual_target_source, re.IGNORECASE
            ):
            ...
            if (
                ...
                targets = [
                    operation.ViTri(
                        ...
                        khoan=at.get("clause"),
                        diem=at.get("point"),
                        so_hieu_van_ban=default_so_hieu,
                    )
                ]
...
def merge_vitri(child, parent):
    if not child.dieu and parent:
        child.dieu = parent.dieu
    if not child.khoan and parent:
        child.khoan = parent.khoan
    if not child.diem and parent:
        child.diem = parent.diem
    if not child.so_hieu_van_ban and parent and parent.so_hieu_van_ban:
        child.so_hieu_van_ban = parent.so_hieu_van_ban
    return child
...
def is_non_action_container(item: dict, instruction: str) -> bool:
    ...
    if item.get("source_clause") is None:
        if DIEU_TITLE_PATTERN.match(text):
            return True
        if re.match(
            r"^(?:Bổ sung|Sửa đổi|Thay thế|Bãi bỏ|Bỏ)\b.*(?:một số|kèm theo|phụ lục)",
            text,
            re.IGNORECASE,
        ):
            return True
    return bool(
        re.match(
            ...
            r"(?:một số|một vài)\b.*\bnhư sau\s*:?\s*$",
            text,
            re.IGNORECASE,
        )
    )
...
def build_created_units(
    replacement_tree: dict,
    root_level: str,
    target_document: str,
    existing_target=None,
    created_targets=None,
) -> list[dict]:
    ...
    if created_targets:
        ...
        for target in created_targets:
            if not target.dieu:
                target.dieu = article
            if not target.khoan and target.diem:
                target.khoan = clause
            level = _target_level_cua(target)
            number = target.diem or target.khoan or target.dieu
            result.append(
                {"unit_id": target.target_id(), "unit_level": level, "number": number}
            )
        return result
    result = []
    for level, node in nodes:
        ...
        if level == "ARTICLE":
            ...
            vt = operation.ViTri(
                dieu=article, khoan=number, so_hieu_van_ban=target_document
            )
        else:
            vt = operation.ViTri(
                dieu=article, khoan=clause, diem=number, so_hieu_van_ban=target_document
            )
        result.append(
            {"unit_id": vt.target_id(), "unit_level": level, "number": number}
        )
    return result


def restrict_replacement_tree_to_created(
    tree: dict, root_level: str, created_targets
) -> tuple[dict, str]:
    ...
    wanted = {
        (_target_level_cua(t), str(t.diem or t.khoan or t.dieu))
        for t in created_targets
    }
    if root_level == "CLAUSE_LIST":
        nodes = [
            n
            for n in tree.get("clauses", [])
            if ("CLAUSE", str(n.get("number"))) in wanted
        ]
    ...
    if root_level == "CLAUSE":
        ...
        return (
            ({"clause": n}, "CLAUSE")
            if ("CLAUSE", str(n.get("number"))) in wanted
            else ({"clauses": []}, "CLAUSE_LIST")
        )
    if root_level == "POINT_LIST":
        nodes = [
            n
            for n in tree.get("points", [])
            if ("POINT", str(n.get("number"))) in wanted
        ]
    ...
    if root_level == "POINT":
        ...
        return (
            ({"point": n}, "POINT")
            if ("POINT", str(n.get("number"))) in wanted
            else ({"points": []}, "POINT_LIST")
        )
    if root_level == "ARTICLE":
        n = tree.get("article") or {}
        if n.get("number") is None and len(created_targets) == 1:
            n = dict(n)
            n["number"] = created_targets[0].dieu
        return {"article": n}, "ARTICLE"
    return tree, root_level


def resolve_replacement_references(
    tree: dict, target, target_document: str
) -> list[dict]:
    ...
    for text in texts:
        result.extend(
            asdict(x)
            for x in reference.resolve_references(text, current, target_document)
        )
    return result
...
def _split_top_level_raw(paragraphs):
    ...
    for p in paragraphs:
        ...
        if m and p.dam and not is_inside_quote:
            ...
            if current_block:
                current_block["paragraphs"].append(p)
        quote_balance += text.count("“") - text.count("”")
        quote_balance = max(quote_balance, 0)
    if current_block:
        blocks.append(current_block)
...
def record_dedicated(source_file: str, source_so_hieu: str) -> list[dict]:
    ...
    for block in blocks:
        ...
        target_so_hieu = (
            target_meta["number"]
            if target_meta
            else AMENDMENT_TARGET_FALLBACK.get(source_so_hieu)
        )
        if not target_so_hieu:
            continue

        targets_block = operation.detect_target(
            clean_tieu_de, default_so_hieu=target_so_hieu
        )
        ...
        for item in source_items:
            ...
            if targets_block:
                base_target = ViTri(
                    targets_block[0].dieu, targets_block[0].khoan, targets_block[0].diem
                )
            ...
            for act in actions:
                ...
                if act["anchor"] and act["anchor"].get("target"):
                    ...
                    if not a_t["article"]:
                        a_t["article"] = base_target.dieu
                    if not a_t["clause"]:
                        a_t["clause"] = base_target.khoan
                    for t in act["targets"]:
                        if not t.dieu and a_t["article"]:
                            t.dieu = a_t["article"]
                        if not t.khoan and a_t["clause"]:
                            t.khoan = a_t["clause"]

            if (
                item["source_clause"]
                and not item["source_point"]
                and actions
                and actions[0]["targets"]
            ):
            ...
            first_target = (
                actions[0]["targets"][0] if actions and actions[0]["targets"] else None
            )
            ...
            repl_tree, root_lvl = build_replacement_tree_and_root(
                item["quote_block"], level_hint
            )
            is_text_or_app = any(
                a["operation"]
                in ("BO_SUNG_TEXT", "BAI_BO_TEXT", "THAY_THE_TEXT", "THAY_THE_PHU_LUC")
                or a.get("text_amendment")
                or a.get("appendix_amendment")
                for a in actions
            )
            if (
                not item["quote_block"]
                and actions[0]["operation"] != "BAI_BO"
                and not is_text_or_app
            ):
                continue

            source_context = {
                "article": {"id": None, "number": block["so"]} if block["so"] else None,
                "clause": {"id": None, "number": item["source_clause"]}
                if item["source_clause"]
                else None,
                "point": {"id": None, "number": item["source_point"]}
                if item["source_point"]
                else None,
            }
            ...
            source_unit_id = (
                (p_ctx.get("id") if p_ctx else None)
                or (c_ctx.get("id") if c_ctx else None)
                or (a_ctx.get("id") if a_ctx else None)
            )
            ...
            for act in actions:
                ...
                is_insert = bool(
                    act.get("anchor")
                    and act["anchor"].get("relation") in ("BEFORE", "AFTER")
                )

                if is_app:
                    act["appendix_amendment"]["new_appendix"]["document"] = (
                        source_so_hieu
                    )
                    act_targets = [
                        {
                            "target_type": "APPENDIX",
                            "appendix_number": act["appendix_amendment"][
                                "old_appendix"
                            ]["number"],
                            "document": act["appendix_amendment"]["old_appendix"][
                                "document"
                            ],
                        }
                    ]
                    ...
                    if act["operation"] == "BAI_BO" or is_text or is_insert:
                        mapped_targets = [
                            {
                                ...
                                "target_context": {
                                    ...
                                    "article": {"id": None, "number": t.dieu}
                                    if t.dieu
                                    else None,
                                    "clause": {"id": None, "number": t.khoan}
                                    if t.khoan
                                    else None,
                                    "point": {"id": None, "number": t.diem}
                                    if t.diem
                                    else None,
                                },
                        ...
                        mapped_targets = match_targets(
                            act["targets"], repl_tree, root_lvl
                        )
                    ...
                    for m in mapped_targets:
                        ...
                        t_unit = (
                            ...
                            or (c_ctx.get("id") if c_ctx else None)
                            or (a_ctx.get("id") if a_ctx else None)
                            or m["target_unit_mock"].target_id()
                        )
                        act_targets.append(
                            {
                                "target_unit": t_unit,
                ...
                created_units = (
                    ...
                    if is_insert and repl_tree
                    else []
                )
                replacement_references = resolve_replacement_references(
                    repl_tree, first_existing, target_so_hieu
                )
            ...
            event_items.append(
                {
                    ...
                    "replacement_tree": repl_tree
                    if any(
                        a["operation"] != "BAI_BO"
                        and not a.get("text_amendment")
                        and not a.get("appendix_amendment")
                        for a in actions
                    )
                    else None,
        ...
        if event_items:
            ev_context = {
                "article": {
                    "id": f"{source_so_hieu.replace('/', '_')}_D{block['so']}",
                    "number": block["so"],
                }
                if block["so"]
                else None,
...
def record_omnibus(source_file: str, source_so_hieu: str) -> list[dict]:
    ...
    for block in blocks:
        ...
        targets_block = operation.detect_target(
            block["tieu_de"], default_so_hieu=target_so_hieu
        )
        pars = [DoanVan(text=block["tieu_de"], dam=True)] + block["paragraphs"]
        source_items = parse_amendment_workflow(pars)
