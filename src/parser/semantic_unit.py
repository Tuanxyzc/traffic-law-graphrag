...
from src.parser.models import Chuong, Dieu, DonViNguNghia, Khoan, VanBan, ViTri

CHUNK_BY_DIEM_LOAI = {"NghiDinh", "NghiDinh_suaDoi"}


def _dien_so_hieu_cho_doi_tuong(
    doi_tuong: list[ViTri], mac_dinh: str | None
) -> list[ViTri]:
    ...
    return [
        v
        if v.so_hieu_van_ban
        else ViTri(dieu=v.dieu, khoan=v.khoan, diem=v.diem, so_hieu_van_ban=mac_dinh)
        for v in doi_tuong
    ]
...
def merge_vitri_child_parent(child: ViTri, parent: ViTri) -> ViTri:
    ...
    res = deepcopy(child)
    if not res.dieu and parent.dieu:
        res.dieu = parent.dieu
    if not res.khoan and parent.khoan:
        res.khoan = parent.khoan
    if not res.diem and parent.diem:
        res.diem = parent.diem
    if not res.so_hieu_van_ban and parent.so_hieu_van_ban:
        res.so_hieu_van_ban = parent.so_hieu_van_ban
    return res
...
def _build_unit(
    ...
    id_: str,
    level: int,
    hanh_dong_dieu: str | None,
    doi_tuong_dieu: list[ViTri],
    noi_dung_rieng: str,
    noi_dung_day_du: str,
    dieu: Dieu,
    khoan: Khoan | None,
    ...
    if doi_tuong_rieng:
        if doi_tuong_dieu:
            ...
            doi_tuong = [
                merge_vitri_child_parent(t, base_target) for t in doi_tuong_rieng
            ]
        else:
    ...
    return DonViNguNghia(
        ...
        noi_dung_goc=noi_dung_day_du,
        noi_dung_chuan_hoa=noi_dung_chuan_hoa,
...
def build_for_dieu_list(
    dieu_list: list[Dieu],
    so_hieu: str,
    loai: str,
    chuong: Chuong | None = None,
    target_so_hieu_mac_dinh: str | None = None,
) -> list[DonViNguNghia]:
    ...
    for dieu in dieu_list:
        ...
        if dieu.trang_thai == "bai_bo":
            ...
            units.append(
                DonViNguNghia(
                    id=f"{dieu.id}_BAIBO",
                    vi_tri=ViTri(dieu=dieu.so, so_hieu_van_ban=so_hieu),
                    hanh_dong="BAI_BO",
                    level=2,
                    noi_dung_goc=noi_dung,
                    noi_dung_chuan_hoa=noi_dung,
                    noi_dung=noi_dung,
                    hieu_luc_tu=dieu.hieu_luc_tu,
                    tieu_de_dieu=dieu.tieu_de,
                    phien_ban=dieu.phien_ban,
                    sua_doi_boi_van_ban=dieu.sua_doi_boi_van_ban,
                )
        ...
        doi_tuong_dieu = (
            operation.detect_target(dieu.tieu_de or "") if hanh_dong_dieu else []
        )
        ...
        for khoan in dieu.khoan:
            if khoan.trang_thai == "bai_bo":
                ...
                units.append(
                    DonViNguNghia(
                        ...
                        vi_tri=ViTri(
                            dieu=dieu.so, khoan=khoan.so, so_hieu_van_ban=so_hieu
                        ),
                        hanh_dong="BAI_BO",
                        level=3,
                        noi_dung_goc=noi_dung,
                        noi_dung_chuan_hoa=noi_dung,
                        noi_dung=noi_dung,
                        hieu_luc_tu=khoan.hieu_luc_tu or dieu.hieu_luc_tu,
                        tieu_de_dieu=dieu.tieu_de,
                        phien_ban=dieu.phien_ban,
                        sua_doi_boi_van_ban=dieu.sua_doi_boi_van_ban,
                    )
            ...
            if chunk_by_diem and khoan.diem:
                clause_instruction = (
                    khoan.noi_dung.splitlines()[0] if khoan.noi_dung else ""
                )
                hanh_dong_khoan = operation.detect_operation(clause_instruction)
                doi_tuong_khoan = (
                    operation.detect_target(clause_instruction)
                    if hanh_dong_khoan
                    else []
                )
...
def build(
    van_ban: VanBan, target_so_hieu_mac_dinh: str | None = None
) -> list[DonViNguNghia]:
    """Duyệt toàn bộ cây VanBan, sinh danh sách Semantic Unit phẳng."""
    units: list[DonViNguNghia] = []
    units.extend(
        build_for_dieu_list(
            van_ban.dieu_khong_chuong,
            van_ban.so_hieu,
            van_ban.loai,
            chuong=None,
            target_so_hieu_mac_dinh=target_so_hieu_mac_dinh,
        )
    )
