...
class ResolvedTarget:
    ...
    def as_vitri(self) -> ViTri:
        return ViTri(dieu=self.article, khoan=self.clause, diem=self.point, so_hieu_van_ban=self.document)
...
P_DIEM = re.compile(r'điểm\s+([a-zđ]\d*(?:\s*(?:,|và)\s*(?:điểm\s+)?[a-zđ]\d*)*)', re.IGNORECASE)
P_KHOAN = re.compile(r'khoản\s+(\d+[a-zđ]?(?:\s*(?:,|và)\s*(?:khoản\s+)?\d+[a-zđ]?)*)', re.IGNORECASE)
...
def resolve_targets(text: str, parent_context=None, default_document=None) -> list[ResolvedTarget]:
    ...
    groups = []
    curr = TargetSpec(document=doc_to_use, article=None, clauses=[], points=[], source_span=None)
    ...
    for i in range(len(groups)):
        ...
        if not groups[i].clauses and not groups[i].article:
            for j in range(i+1, len(groups)):
                if groups[j].clauses:
                    groups[i].clauses = [groups[j].clauses[0]] if groups[j].clauses else []
                    break
    ...
    if parent_context:
        for r in resolved:
            if not r.article and parent_context.dieu: r.article = parent_context.dieu
            if not r.clause and parent_context.khoan: r.clause = parent_context.khoan
            ...
            if (not r.point and not r.clause and not r.article
                    and parent_context.diem):
