def validate_version_timeline(provision_id, versions, valid_action_ids=None):
    ...
    for index, version in enumerate(versions):
        ...
        if version.effective_status == "EXTERNAL" and not version.external_rule_ids:
            raise ValueError(f"EXTERNAL version missing external_rule_ids: {version.version_id}")
        if valid_action_ids is not None and version.produced_by is not None and version.produced_by not in valid_action_ids:
            raise ValueError(f"Invalid produced_by: {version.version_id}")
        if index:
            previous = versions[index - 1]
            if previous.valid_from and version.valid_from and previous.valid_from > version.valid_from:
                raise ValueError(f"Timeline not sorted: {provision_id}")
            if previous.valid_to is not None and version.valid_from is not None and previous.valid_to != version.valid_from:
