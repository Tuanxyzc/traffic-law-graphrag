...
from src.parser.metadata_versioning import (
...
from src.parser.semantic_unit import _build_unit, build_for_dieu_list
from src.parser.structure import parse_document, parse_khoan_list
...
class InsertAnchorRegressionTests(unittest.TestCase):
    def assert_insert(self, instruction, quote, relation, existing_id, created_id):
        ...
        tree, root = build_replacement_tree_and_root(
            quote, "POINT" if existing.diem else "CLAUSE"
        )
        ...
        self.assertIn(
            "trước" if relation == "BEFORE" else "sau", instruction[start:end]
        )
    ...
    def test_insert_before_point(self):
        self.assert_insert(
            ...
            "c) Nội dung mới.",
            "BEFORE",
            "168_2024_ND-CP_D10_K3_Dd",
            "168_2024_ND-CP_D10_K3_Dc",
        )
    ...
    def test_text_amendment_ignores_structural_words_inside_quotes(self):
        action = parse_actions(
            ...
            "tại điểm a khoản 6 Điều 14.",
            DOC,
        )[0]
        self.assertEqual(
            [t.target_id() for t in action["targets"]], ["168_2024_ND-CP_D14_K6_Da"]
        )
    ...
    def test_text_amendment_with_multiple_quoted_spans(self):
        ...
        self.assertEqual(
            [t.target_id() for t in action["targets"]], ["168_2024_ND-CP_D47_K3_Dh"]
        )

    def test_text_deletion_ignores_deleted_structural_phrase(self):
        action = parse_actions(
            'Bỏ cụm từ "Điểm g khoản 5;" tại điểm a khoản 1 Điều 48.',
            DOC,
        )[0]
        self.assertEqual(
            [t.target_id() for t in action["targets"]], ["168_2024_ND-CP_D48_K1_Da"]
        )
    ...
    def test_quote_boundary_anomaly_still_resolves_multiple_targets(self):
        ...
        self.assertEqual(
            [t.target_id() for t in action["targets"]],
            [
    ...
    def test_unresolved_instruction_is_not_dropped(self):
        ...
        self.assertEqual(
            action["raw_instruction"], '6. Bổ sung cụm từ "abc" vào sau cụm từ "xyz".'
        )

    def test_multiple_structural_targets(self):
        action = parse_actions(
            "Sửa đổi khoản 1, khoản 3 và khoản 6 Điều 25.",
            DOC,
        )[0]
    ...
    def test_coordinated_actions_inherit_single_shared_article_context(self):
        actions = parse_actions(
            ...
            "khoản 18 Điều 9 như sau:",
            "36/2024/QH15",
        )
        self.assertEqual(actions[0]["targets"][0].target_id(), "36_2024_QH15_D9_K18")
        self.assertEqual(actions[0]["raw_instruction"], "2. Sửa đổi, bổ sung khoản 18")

    def test_explicit_external_document(self):
        action = parse_actions(
            "Sửa đổi khoản 3, khoản 4 Điều 27 Nghị định số 184/2025/NĐ-CP.",
            DOC,
        )[0]
        self.assertEqual(
            [t.so_hieu_van_ban for t in action["targets"]],
    ...
    def test_mixed_grouped_locations_do_not_bleed_context(self):
        action = parse_actions(
            ...
            "Điều 18, khoản 1, khoản 3 và khoản 6 Điều 25.",
            DOC,
        )[0]
        self.assertEqual(
            [t.target_id() for t in action["targets"]],
    ...
    def test_document_title_does_not_create_false_second_action(self):
        ...
        self.assertEqual(
            [t.target_id() for t in actions[0]["targets"]],
            [
    ...
    def test_appendix_replacement_has_no_article_target(self):
        action = parse_actions(
            ...
            "151/2024/NĐ-CP bằng Phụ lục I ban hành kèm theo Nghị định này.",
            DOC,
        )[0]
        ...
        refs = resolve_replacement_references(
            {
                "clause": {
                    "number": "2",
                    "content": "Thực hiện theo khoản này của Nghị định này.",
                }
            },
    ...
    def test_omnibus_semantic_targets_are_selected_per_article(self):
        article = SimpleNamespace(
            ...
            noi_dung="Sửa đổi khoản 1 Điều 9.",
            khoan=[],
            hieu_luc_tu=None,
            phien_ban="goc",
            sua_doi_boi_van_ban=None,
        ...
        self.assertEqual(units[0].doi_tuong[0].so_hieu_van_ban, "36/2024/QH15")
    ...
    def test_cross_document_clause_reference(self):
        ...
        self.assertEqual(refs[0].gia_tri_xac_dinh.target_id(), "36_2024_QH15_D64_K1")
    ...
    def test_internal_absolute_clause_reference(self):
        ...
        self.assertEqual(refs[0].gia_tri_xac_dinh.target_id(), "35_2024_QH15_D24_K3")

    def test_point_unit_inherits_clause_amendment_target(self):
        point = SimpleNamespace(
            so="c", id="238_2026_ND-CP_D8_K3_Dc", noi_dung="c) Nội dung mới"
        )
        clause = SimpleNamespace(
            so="3",
            id="238_2026_ND-CP_D8_K3",
            trang_thai="hieu_luc",
        ...
        self.assertEqual(units[0].doi_tuong[0].target_id(), "168_2024_ND-CP_D21_K5_Dc")

    def test_title_content_are_not_merged(self):
        paragraphs = [
            DoanVan("Điều 1. Phạm vi điều chỉnh", True),
        ...
        self.assertEqual(
            parsed.dieu_khong_chuong[0].noi_dung, "Nghị định này quy định chi tiết."
        )
        self.assertTrue(
            is_non_action_container(
