...
from src.versioning.version_models import ProvisionVersion
from src.versioning.version_validator import (
    ...
    return {
        "doc_1": [
            MockNode(
                id="doc_1_D1",
                label="Article",
                properties={"number": "1", "text": "Old text 1"},
            ),
            MockNode(
                id="doc_1_D2",
                label="Article",
                properties={"number": "2", "text": "Old text 2"},
            ),
        ]
    }
    ...
    return [
        {
            ...
            "operation": "SUA_DOI",
            "targets": [
                {"target_unit": "doc_1_D1", "replacement_path": ["articles", 0]}
            ],
        ...
        },
        {
            ...
            "operation": "BO_SUNG",
            "created_units": [
                {"unit_id": "doc_1_D3", "unit_level": "ARTICLE", "number": "3"}
            ],
            "replacement_tree": {
                "articles": [{"number": "3", "text": "Brand new text 3"}]
            },
        },
    ]


def test_version_builder_full_lifecycle(
    mock_structure, mock_amendment_actions, mock_effective_rules
):
